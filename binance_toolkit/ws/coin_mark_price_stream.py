...
class MarkPriceStreamWriter:
    ...
    def _writer_loop(self) -> None:
        ...
        while not self._stop_event.is_set() or not self._queue.empty():
            ...
            if time.time() - last_stats_time >= stats_interval:
                ...
                logger.info(
                    "币本位写入器统计 | 接收: %d | 采样: %d (%s) | 跳过: %d | 写入: %d | 失败: %d | 队列: %d",
    ...
    def _flush_buffer(self, buffer: list[dict]) -> None:
        ...
        logger.info(
            "开始写入 %d 条数据, 涉及 %d 个合约: %s",
            len(buffer), len(symbols), ", ".join(sorted(symbols)),
        )
    ...
    if isinstance(data, list):
        ...
        if data:
            ...
            if event_time_ms:
                ...
                print(f"\n[{now_str}] 收到 {len(data)} 条标记价格更新 (数据延迟: {delay:.1f}s):")
            else:
                print(f"\n[{now_str}] 收到 {len(data)} 条标记价格更新:")
        for item in data:
            _print_single(item)
    else:
        ...
        if event_time_ms:
            event_time = datetime.fromtimestamp(event_time_ms / 1000, tz=timezone.utc)
            delay = (now - event_time).total_seconds()
            print(f"\n[{now_str}] 标记价格更新 (数据延迟: {delay:.1f}s):")
        else:
            print(f"\n[{now_str}] 标记价格更新:")
        _print_single(data)
